...
def readResultsFromFile(pers,hos,ambs,filename):
  ...
  return readresults(pers,hos,ambs,result)
...
#              print 'Time', amb.time,"@",loc,": ERROR: Person gets picked or rescued on other path (persId,time,ambuId,loc)="
              break
              #raise IllegalPlanError(errmsg)
...
#  del sys.argv[1]
  readResultsFromFile(persons, hospitals, ambulances,sys.argv[2])
#  for i in range(len(hospitals)):
